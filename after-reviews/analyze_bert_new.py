...
for key in keys:
    ...
    for repeat in range(n_repeats):
        y_repeat = y[repeat]
        proba = np.load("probas_bert/%i_%s_new_struct.npy" % (repeat, key))
        pred = np.argmax(proba, axis=2)
...
for key in keys:
    ...
    for repeat in range(n_repeats):
        y_repeat = y[repeat]
        proba_words = np.load("probas_bert/%i_%s_new.npy" % (repeat, key))
        proba_struct = np.load("probas_bert/%i_%s_new_struct.npy" % (repeat, key))
...
for repeat in range(n_repeats):
    ...
    for key in keys:
        proba_words = np.load("probas_bert/%i_%s_new.npy" % (repeat, key))
        proba_struct = np.load("probas_bert/%i_%s_new.npy" % (repeat, key))
        proba = (proba_words + proba_struct)/2
        probas.append(proba)
    probas = np.array(probas)
